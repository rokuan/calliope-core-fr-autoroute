...
/**
  ...
  */
object WordRules {
  def word(w: WordType.Value*): TerminalState[Word] = new TerminalMatcher(new SimpleWordMatcherBuilder().setTypes(w: _*).build())

  def word(v: String): TerminalState[Word] = new TerminalMatcher(new SimpleWordMatcherBuilder().setRegex(v).build())

  def word(v: String, w: WordType.Value*): TerminalState[Word] = new TerminalMatcher(new SimpleWordMatcherBuilder()
    .setTypes(w: _*)
    .setRegex(v)
    .build())
}
